package com.example.deckflowapp_android.di

import com.example.deckflowapp_android.api.CardAPIService
import com.example.deckflowapp_android.api.UserAPIService
import com.example.deckflowapp_android.repository.CardRepository
import com.example.deckflowapp_android.repository.ICardRepository
import com.example.deckflowapp_android.repository.ILoginRepository
import com.example.deckflowapp_android.repository.LoginRepository
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.components.SingletonComponent
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
class RepositoryModule {
    private var loginRepository: ILoginRepository? = null
    private var cardRepository: ICardRepository? = null

    @Provides
    @Singleton
    fun provideLoginRepository(userApiService: UserAPIService): ILoginRepository {
        if (loginRepository == null) {
            loginRepository = LoginRepository(userApiService)
        }

        return loginRepository!!
    }

    @Provides
    @Singleton
    fun provideCardRepository(cardApiService: CardAPIService): ICardRepository {
        if (cardRepository == null) {
            cardRepository = CardRepository(cardApiService)
        }

        return cardRepository!!
    }
}